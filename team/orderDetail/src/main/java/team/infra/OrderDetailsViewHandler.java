...
public class OrderDetailsViewHandler {
    ...
    public void whenOrdered_then_CREATE_1(@Payload Ordered ordered) {
        try {
            ...
            orderDetails.setTotalPrice(
                Double.valueOf(ordered.getOrderTotalPrice())
            );
            // view 레파지 토리에 save
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenPaymentApproved_then_UPDATE_2(
        ...
        try {
            ...
            for (OrderDetails orderDetails : orderDetailsList) {
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                orderDetails.setPayStatus(paymentApproved.getPayStatus());
                // view 레파지 토리에 save
                orderDetailsRepository.save(orderDetails);
            }
        } catch (Exception e) {
    ...
    public void whenPaymentCanceled_then_UPDATE_3(
        ...
        try {
            ...
            for (OrderDetails orderDetails : orderDetailsList) {
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                orderDetails.setPayStatus(paymentCanceled.getPayStatus());
                // view 레파지 토리에 save
                orderDetailsRepository.save(orderDetails);
            }
        } catch (Exception e) {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeliveryCanceled_then_UPDATE_5(
